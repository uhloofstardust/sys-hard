...
def set_password():
    usrnm = entry_usrnm.get()
    pwd = entry_passwd.get()
    if pwd:
        try:
            subprocess.run(["sudo", "chpasswd"], input=f"{usrnm}:{pwd}".encode(), capture_output=True)
...
label_passwd = tk.Label(root, text="Set Password:")
label_passwd.pack()
entry_usrnm = tk.Entry(root)
entry_usrnm.pack()
entry_passwd = tk.Entry(root)
entry_passwd.pack()
set_passwd_button = tk.Button(root, text="Set Password:", command=set_password)
set_passwd_button.pack()
